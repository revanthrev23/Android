...
import com.example.texts.AdapterClasses.ChatsAdapter.ViewHolder
import com.example.texts.MainActivity
import com.example.texts.ModelClasses.Chat
import com.example.texts.R
import com.example.texts.ViewFullImageActivity
...
class ChatsAdapter(mContext: Context, mChatList: List<Chat>, imageUrl: String) :RecyclerView.Adapter<ChatsAdapter.ViewHolder?>(){
    ...
    private fun deleteSentMessages(position: Int, holder: ChatsAdapter.ViewHolder){
        val ref = FirebaseDatabase.getInstance().reference.child("Chats").child(mChatList.get(position).getMessageId()!!).removeValue().addOnCompleteListener { task ->
            if(task.isSuccessful){
                Toast.makeText(holder.itemView.context, "Message Deleted", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(holder.itemView.context, "Message could not be deleted", Toast.LENGTH_LONG).show()
            }
        }
    }
